...
func FuzzDecodeStringSafe(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, input string) {
		decoded, err := DecodeStringSafe(input)
		if err == nil && len(decoded) > 0 {
			reencoded := EncodeToString(decoded)
			redecoded, err2 := DecodeStringSafe(reencoded)
			if err2 != nil {
				t.Errorf("round-trip encode→decode failed: %v", err2)
			}
			if len(decoded) != len(redecoded) {
...
func FuzzDecodeStringSafeNoPadding(f *testing.F) {
	f.Add("jbswy3dp")
	f.Add("")
	f.Add("4oymiquy7qobjgx36tejs35zeqt24qpemsnzgtfeswmrw6csxbkq")
	f.Add("JBSWY3DP")
	...
	f.Fuzz(func(t *testing.T, input string) {
		decoded, err := DecodeStringSafeNoPadding(input)
		if err == nil && len(decoded) > 0 {
			reencoded := EncodeToStringNoPadding(decoded)
			redecoded, err2 := DecodeStringSafeNoPadding(reencoded)
			if err2 != nil {
				t.Errorf("round-trip encode→decode failed: %v", err2)
			}
			if len(decoded) != len(redecoded) {
