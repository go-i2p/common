...
func (c *Certificate) excessBytes() []byte {
	...
	log.WithFields(logger.Fields{"pkg": "certificate", "func": "Certificate.excessBytes"}).Debug("No excess bytes found in certificate")
	return nil
}
...
func (c *Certificate) Bytes() []byte {
	...
	payload, err := c.Data()
	if err != nil {
		log.WithFields(logger.Fields{"pkg": "certificate", "func": "Certificate.Bytes", "reason": "invalid payload"}).Error(err.Error())
...
func (c *Certificate) Data() ([]byte, error) {
	...
	if lenErr != nil {
		log.WithFields(logger.Fields{"pkg": "certificate", "func": "Certificate.Data", "reason": "invalid length"}).Error(lenErr.Error())
		return nil, lenErr
	}
	var src []byte
	if length > len(c.payload) {
		src = c.payload
		log.WithFields(logger.Fields{"pkg": "certificate", "func": "Certificate.Data"}).Warn("Certificate payload shorter than specified length")
	} else {
